var IMDBSystem = (function(THREE){        
	...
	return function(node, width, height, cb) {
		    ...
		    scene.add(cube);
	            var bounding = new THREE.CubeGeometry( range + 200, range + 200, range + 200, 2, 2, 2 ),
			bounding_material = new THREE.MeshBasicMaterial( {color: 0xEEEEEE, wireframe: true, transparent: true, opacity: .4 } ),
			cube = new THREE.Mesh( bounding, bounding_material );
		    scene.add(cube);
