...
The [`lifetime`](http://www.w3.org/TR/IndexedDB/#dfn-transaction-lifetime) of a transactions lasts as long as it's referenced: it's "`active`" so long as it's being referenced, after which it is said to be "`finished`" and the transaction is [`commited`](http://www.w3.org/TR/IndexedDB/#dfn-commit).
